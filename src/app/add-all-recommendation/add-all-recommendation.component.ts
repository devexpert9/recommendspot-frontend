...
export class AddAllRecommendationComponent implements OnInit {
  ...
  uploadImage() {
    ...
    this.apiService.postData(frmData, "uploadReccImage").subscribe(
      (result) => {
        this.image = result;
        localStorage.setItem("previousImage", this.image);
        this.ref.detectChanges();
        this.profileImageSubmit();
      },
      (err) => {
  ...
  profileImageSubmit() {
    ...
    var dict = {
      ...
      user_id: localStorage.getItem("userId"),
      add_user_type: "user",
      web_link_content: this.authForm.value.web_link_content,
      location: this.authForm.value.location,
    ...
    localStorage.setItem("FilledAddRecc", JSON.stringify(dict));
    localStorage.setItem("isFillAddReccStatus", "true");
